...
export async function saveERC721Transfers(context: Context): Promise<void> {
	...
	let token = await get(context.store, ERC721Token, metadatId);
	if (token == null) {
...
export async function saveERC1155SingleTransfers(
	context: Context
): Promise<void> {
	const blockNumber = context.substrate.block.height.toString();
	let timestamp = BigInt(context.substrate.block.timestamp.toString());
	...
	const contractAddress = contractOf(context);
	const indexInBlock = context.substrate.event.indexInBlock;
	let transferId = txHash
		.concat('-'.concat(tokenIdString))
		.concat('-'.concat(indexInBlock.toString()));
	let metadatId = contractAddress + '-' + tokenIdString;
	...
	if (contractData == null) {
		...
		let contractTotalSupply =
			BigInt(tokenIdString) > contractData.totalSupply
	...
	let token = await get(context.store, ERC1155Token, metadatId);
	if (token == null) {
...
export async function saveERC1155MultipleTransfers(
	context: Context
): Promise<void> {
	const blockNumber = context.substrate.block.height.toString();
	let timestamp = BigInt(context.substrate.block.timestamp.toString());
	...
	for (let i = 0; i < tokenIds.length; i++) {
		...
		if (previousOwner.id != '0x0000000000000000000000000000000000000000') {
			senderTokenOwner.balance =
				senderTokenOwner.balance - bigintOf(amount);
		}
